...
function updateCartDisplay() {
	const cartData = localStorage.getItem('cart'); if (!cartData) { return }
	cartList.innerHTML = ''; cartItems.forEach(product => { cartList.insertAdjacentHTML('beforeend', createCartItemHTML(product)) }); basketCounter.textContent = cartItems.length; basketCounterMobile.textContent = cartItems.length
}
cartList.addEventListener('click', (e) => { const productItem = e.target.closest('.cart-list__item'); const isCheckbox = e.target.matches('input[type="checkbox"]'); if (productItem && !isCheckbox) { const productID = productItem.dataset.productId; const confirmation = confirm("Are you sure you want to remove this item from your cart?"); if (confirmation) { const indexToRemove = cartItems.findIndex(item => String(item.id) === productID); if (indexToRemove !== -1) { cartItems.splice(indexToRemove, 1); saveCartToLocalStorage(cartItems); updateCartDisplay() } } } })